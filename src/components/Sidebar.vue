<template>
  <div id="sidebar">
    <div class="header">
      <div class="img">
        <img ref="img" :src="user.avatarUrl" @click="UserInfo" alt />
      </div>
  ...
  methods: {
    ...
    getuser() {
      if (!JSON.parse(sessionStorage.getItem('uid'))) {
        // 默认，无登录时
        this.$refs.img.src = 'https://blog-1259178461.cos.ap-chengdu.myqcloud.com/vue-music/default.jpg'
        ...
        // 取vuex的id获取信息
        this.axios.get('http://localhost:3000/user/detail?uid=' + this.$store.state.uid).then((res) => {
